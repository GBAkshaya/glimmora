-- Glimmora — pre-launch fixes:
-- 1) Cancelling an order never restored the stock it had reserved, so
--    cancelled orders permanently burned inventory. Add a trigger that
--    restores stock_count for every line item the first time an order's
--    status transitions to 'Cancelled'.
-- 2) create_order's percent-discount branch had no upper clamp (unlike the
--    fixed-amount branch, which already used least(...)), so a percent
--    coupon above 100 could drive discount_amount above subtotal. Clamp it
--    the same way the amount branch already is.
-- 3) create_order never checked products.is_active, so a deactivated
--    product with leftover stock_count could still be ordered directly via
--    the API even though it's hidden from the storefront. Now rejected.
-- 4) create_order returns table (id uuid, ...), which makes `id` an implicit
--    variable throughout the function body. Three `where id = ...` checks
--    against products were unqualified, causing every checkout to fail with
--    "column reference "id" is ambiguous". Qualified as products.id.
