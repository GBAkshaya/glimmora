...
export interface CoverflowCarouselProps {
  ...
  /** Fires when the centred card is tapped/clicked (not dragged, not a side card). */
...
export function CoverflowCarousel({
  ...
  /** Whether the current pointer down/up pair moved far enough to count as a
      drag rather than a tap — a tap on the centred card fires onSlideClick. */
  ...
  const endDrag = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    settle(target);
    if (!draggedRef.current && onSlideClick) {
      const index = indexAt(target);
      onSlideClick(slides[index], index);
    }
  };
        ...
        <div
          ...
          onKeyDown={(event) => {
            if (event.key === "ArrowLeft") {
              ...
              event.preventDefault();
              onSlideClick(slides[selected], selected);
            }
          }}
          // Vertical padding keeps the drop shadows clear of the overflow clip.
          className={cn(
            "overflow-hidden py-10 outline-none ring-ring focus-visible:ring-2",
            onSlideClick ? "cursor-pointer active:cursor-grabbing" : "cursor-grab active:cursor-grabbing",
          )}
          style={{
        ...
        {showNavigation && (
          <>
            <button
              ...
              onClick={() => nudge(-1)}
              className="absolute left-3 top-1/2 z-[20] -translate-y-1/2 rounded-full bg-background/70 p-2 text-foreground backdrop-blur transition hover:bg-background"
            >
            ...
            <button
              ...
              onClick={() => nudge(1)}
              className="absolute right-3 top-1/2 z-[20] -translate-y-1/2 rounded-full bg-background/70 p-2 text-foreground backdrop-blur transition hover:bg-background"
            >
