  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] aspect-[2/3] sm:aspect-auto sm:min-h-[600px]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-[220px] sm:w-[260px] md:w-[300px] aspect-[3/4] overflow-hidden rounded-2xl shadow-xl bg-ivory">
                <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="absolute inset-0 w-full h-full object-cover z-10" />
              </div>
            );
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card absolute cursor-pointer block">{image}</a>
            ) : (
